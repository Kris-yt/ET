...
export const t = (
  ...
  if (
    ...
    typeof (target as any)?.$$typeof !== 'symbol'
  ) {
  ...
  if (typeof target === 'object') {
    res = React.cloneElement(
      target,
      {},
      rebuildReactElement(target.props.children, replaceFn),
    )
  }
  ...
  if (__DEV__ && window.__TRANS_OUTPUT__.length > 0) {
    ...
    const output = JSON.stringify(
      _.uniq(
        _.filter(
          window.__TRANS_OUTPUT__,
          (item: string) => !_.includes(keys, item),
        ),
      ),
    )
    if (output !== '[]') {
...
// 递归深度遍历每一个DOM节点
const rebuildReactElement = (children: any, replaceFn: Function): any => {
  ...
  if (typeof children === 'object' && typeof children.$$typeof === 'symbol') {
    return React.cloneElement(
      children,
      { key: `i18n-${children.key || 0}` },
      rebuildReactElement(children.props.children, replaceFn),
    )
  }
  ...
  _.each(children, (item: any, index: number): any => {
    ...
    if (typeof item === 'number') {
      ...
      cloneChildren.push(
        React.cloneElement(
          item,
          { key: `i18n-${item.key || index}` },
          rebuildReactElement(item.props.children, replaceFn),
        ),
      )
...
const replace =
  ...
  (text: string): String => {
    let result =
      _.get(
        language,
        `["${md5(text).substr(0, 16)}"]["${GlobalVar.get('LANGUAGE')}"]`,
      ) || text
    // 变量替换
